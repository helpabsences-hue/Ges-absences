          ...
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
            {sessionLoading ? (
              ...
              students.map((student, idx) => {
                ...
                const status       = record?.status ?? 'unmarked'
                const reason       = record?.reason ?? ''
                const arrival_time = record?.arrival_time ?? ''
                const isAbsent = status === 'absent'
                const isLate   = status === 'late'
                ...
                return (
                  ...
                  >
                    ...
                    {/* Reason + arrival time — shown for absent / late */}
                    {(isAbsent || isLate) && (
                      <div className="mt-2.5 ml-10 pl-8 space-y-2">
                        {/* Arrival time — only for late */}
                        {isLate && (
                          <div className="flex items-center gap-2">
                            <span className="text-xs text-yellow-400 shrink-0">⏱ Heure d'arrivée:</span>
                            <input
                              type="time"
                              value={arrival_time}
                              onChange={(e) => setArrivalTime(student.id, e.target.value)}
                              className="bg-slate-800/60 border border-yellow-500/30 rounded-lg px-3 py-1.5 text-xs text-white focus:outline-none focus:ring-1 focus:ring-yellow-500 transition"
                            />
                            {arrival_time && activePlanning?.start_time && (
                              <span className="text-xs text-yellow-400">
                                {(() => {
                                  const [ah, am] = arrival_time.split(':').map(Number)
                                  const [sh, sm] = activePlanning.start_time.split(':').map(Number)
                                  const diff = (ah * 60 + am) - (sh * 60 + sm)
                                  return diff > 0 ? `${diff} min de retard` : ''
                                })()}
                              </span>
                            )}
                          </div>
                        ...
                        <input
                          ...
                          value={reason}
                          onChange={(e) => setReason(student.id, e.target.value)}
                          placeholder={isAbsent ? 'Raison de l\'absence (optionnel)' : 'Raison du retard (optionnel)'}
                          className="w-full bg-slate-800/60 border border-slate-700 rounded-lg px-3 py-2 text-xs text-white placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-blue-500 transition"
                        />
                      </div>
                    )}
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={async () => {
                  ...
                  // Auto-close after successful save and refresh status
