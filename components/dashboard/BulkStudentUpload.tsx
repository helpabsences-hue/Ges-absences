...
export default function BulkStudentUpload() {
  ...
  const processRows = async (rows: FileRow[]) => {
    ...
    if (remaining <= 0) {
      toast.error(`Limite du plan ${plan} atteinte (${limit} élèves). Contactez-nous pour upgrader.`)
      return { inserted: 0, skipped: [], errors: [`Limite ${plan} atteinte: ${current}/${limit} élèves`] }
    }
    ...
    rows.forEach((row, i) => {
      ...
      if (toInsert.length >= remaining) {
        skipped.push(`Ligne ${i + 2}: limite du plan ${plan} atteinte (${limit} élèves max)`)
        return
      }
