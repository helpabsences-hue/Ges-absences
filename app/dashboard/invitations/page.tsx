...
export default function InvitationsPage() {
  ...
  // Auto-process any pending invitations on page load
  useEffect(() => {
    fetch('/api/invite-batch', { method: 'POST' })
      .then(r => r.json())
      .then(data => {
        if (data.sent > 0) fetchStudents()
      })
      .catch(console.error)
  }, [])
          ...
          <button
            onClick={() => {
              fetch('/api/invite-batch', { method: 'POST' })
                .then(r => r.json())
                .then(data => {
                  if (data.sent > 0) { fetchStudents(); toast.success(`${data.sent} email(s) envoyé(s)`) }
                  else toast.success('Aucun email en attente')
                })
            }}
            className="flex items-center gap-1.5 text-xs font-medium text-slate-400 hover:text-blue-400 hover:bg-blue-500/10 px-3 py-1.5 rounded-lg transition border border-slate-700">
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
            </svg>
