...
export default function StudentsPage() {
  ...
  const handleBulkDelete = async () => {
    ...
    const confirm = window.confirm(`Supprimer ${selected.size} étudiant(s) ?`)
    if (!confirm) return
    setDeleting(true)
  ...
  return (
    <div className={`max-w-6xl mx-auto space-y-4 sm:space-y-6 ${isRtl ? 'text-right' : ''}`}>
      ...
      {/* ── Bulk delete bar ────────────────────────────── */}
      {selected.size > 0 && (
        <div className="bg-blue-600/10 border border-blue-500/30 rounded-xl px-4 py-3 flex items-center justify-between gap-3">
