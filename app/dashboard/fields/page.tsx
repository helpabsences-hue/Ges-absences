...
const UI: Record<Lang, {
  ...
  confirmDelete: string; deleteLabel: string; cancelLabel: string
}> = {
  fr: {
    ...
    saving: 'Enregistrement…', update: 'Modifier', add: 'Ajouter', cancel: 'Annuler', deleteLabel: 'Supprimer', cancelLabel: 'Annuler',
  ...
  en: {
    ...
    saving: 'Saving…', update: 'Update Field', add: 'Add Field', cancel: 'Cancel', deleteLabel: 'Delete', cancelLabel: 'Cancel',
  ...
  ar: {
    ...
    errName: 'الاسم مطلوب.',
    saving: 'جارٍ الحفظ…', update: 'تعديل', add: 'إضافة', cancel: 'إلغاء', deleteLabel: 'حذف', cancelLabel: 'إلغاء',
...
export default function FieldsPage() {
  ...
  const handleBulkDelete = async () => {
    if (!selected.size || !true) return
    setDeleting(true)
    for (const id of selected) await deleteField(id)
    ...
    <div className={`max-w-6xl mx-auto space-y-4 sm:space-y-6 ${isRtl ? 'text-right' : ''}`}>
      ...
      {selected.size > 0 && (
        <div className="bg-blue-600/10 border border-blue-500/30 rounded-xl px-4 py-3 flex flex-wrap items-center justify-between gap-3">
          <p className="text-sm font-medium text-blue-400">{selected.size} filière{selected.size > 1 ? 's' : ''} sélectionnée{selected.size > 1 ? 's' : ''}</p>
          <div className="flex items-center gap-2">
            <button onClick={() => setSelected(new Set())} className="text-xs text-slate-400 hover:text-white px-3 py-1.5 rounded-lg hover:bg-slate-800 transition">Annuler</button>
            <button onClick={() => setConfirmOpen(true)} disabled={deleting}
              className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-red-600 hover:bg-red-500 text-white transition disabled:opacity-50">
              {deleting ? ui.deleteLabel + '…' : `🗑 ${ui.deleteLabel} (${selected.size})`}
            </button>
          </div>
        </div>
      )}
        ...
        <div className="space-y-3 sm:space-y-4">
          ...
          {fields.length > 0 && (
            <div className="flex items-center gap-2 px-1">
              <input type="checkbox"
                checked={fields.every(f => selected.has(f.id))}
                onChange={() => toggleSelectAll(fields.map(f => f.id))}
          ...
          {fields.map((field) => {
            ...
            return (
              <div key={field.id}
                className={`bg-slate-900 border hover:border-slate-700 rounded-2xl p-4 sm:p-5 transition group ${selected.has(field.id) ? 'border-blue-500/40 bg-blue-600/5' : 'border-slate-800'}`}>
                <div className={`flex items-start gap-4 ${isRtl ? 'flex-row-reverse' : ''}`}>
                  {/* Checkbox */}
                  <input type="checkbox"
                    checked={selected.has(field.id)}
                    onChange={() => toggleSelect(field.id)}
                    className="w-4 h-4 rounded border-slate-600 bg-slate-800 text-blue-600 cursor-pointer mt-0.5 shrink-0"/>
      ...
      <ConfirmDialog
        open={confirmOpen}
        title={`Supprimer les filières (${selected.size})`}
        message="Cette action est irréversible."
        confirmLabel={ui.deleteLabel}
