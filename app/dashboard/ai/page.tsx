            ...
            <div className="space-y-4" style={{ minHeight: '400px' }}>
              ...
              <div className={`flex items-center justify-between gap-3 ${isRtl ? 'flex-row-reverse' : ''}`}>
                ...
                <button onClick={runPrediction} disabled={predicting}
                  className="flex items-center gap-2 bg-blue-700 hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed
              ...
              {!predicting && prediction && (
                <div className="space-y-4">
                  ...
                  {prediction.students?.filter((s: any) => s.risk_level !== 'low').length > 0 && (
                    <div className="space-y-2">
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t.watchTitle}</p>
                      {prediction.students
                        .filter((s: any) => s.risk_level !== 'low')
                        .slice(0, 6)
                        .map((s: any, i: number) => (
