...
const UI: Record<Lang, {
  ...
  en: {
    ...
    saving: 'Saving…', update: 'Update Course', add: 'Add Course', cancel: 'Cancel', deleteLabel: 'Delete', cancelLabel: 'Cancel',
    ...
    <div className={`max-w-6xl mx-auto space-y-4 sm:space-y-6 ${isRtl ? 'text-right' : ''}`}>
      ...
      {selected.size > 0 && (
        <div className="bg-blue-600/10 border border-blue-500/30 rounded-xl px-4 py-3 flex flex-wrap items-center justify-between gap-3">
          <p className="text-sm font-medium text-blue-400">{selected.size} matière{selected.size > 1 ? 's' : ''} sélectionnée{selected.size > 1 ? 's' : ''}</p>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          ...
          {filtered.map((course, index) => {
            ...
            return (
              <div key={course.id}
                className={`bg-slate-900 border rounded-2xl p-4 sm:p-5 flex flex-col gap-3
                  group hover:border-slate-700 transition ${selected.has(course.id) ? 'border-blue-500/40 bg-blue-600/5' : 'border-slate-800'}`}>
      ...
      <ConfirmDialog
        open={confirmOpen}
        title={`Supprimer les matières (${selected.size})`}
