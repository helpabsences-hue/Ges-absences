...
const UI: Record<Lang, {
  ...
  confirmDelete: string; deleteLabel: string; cancelLabel: string; noSlotsDay: string
}> = {
  fr: {
    ...
    saving: 'Enregistrement…', add: 'Ajouter', cancel: 'Annuler', deleteLabel: 'Supprimer', cancelLabel: 'Annuler',
  ...
  en: {
    ...
    saving: 'Saving…', add: 'Add Slot', cancel: 'Cancel', deleteLabel: 'Delete', cancelLabel: 'Cancel',
  ...
  ar: {
    ...
    saving: 'جارٍ الحفظ…', add: 'إضافة', cancel: 'إلغاء', deleteLabel: 'حذف', cancelLabel: 'إلغاء',
...
export default function PlanningPage() {
  ...
  const handleBulkDelete = async () => {
    ...
    setDeleting(true)
    for (const id of selected) await deleteSlot(id)
  ...
  return (
    <div className={`max-w-6xl mx-auto space-y-4 sm:space-y-6 ${isRtl ? 'text-right' : ''}`}>
        ...
        <>
          ...
          {selected.size > 0 && (
            <div className="bg-blue-600/10 border border-blue-500/30 rounded-xl px-4 py-3 flex flex-wrap items-center justify-between gap-3">
              <p className="text-sm font-medium text-blue-400">{selected.size} créneau{selected.size > 1 ? 'x' : ''} sélectionné{selected.size > 1 ? 's' : ''}</p>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full">
                        <thead>
                          <tr className="border-b border-slate-800/60">
                            <th className="px-4 py-3 w-10">
                              <input type="checkbox"
                                checked={daySlots.length > 0 && daySlots.every(s => selected.has(s.id))}
                                onChange={() => toggleSelectAll(daySlots.map(s => s.id))}
                        ...
                        <tbody className="divide-y divide-slate-800/60">
                          {daySlots.map((slot) => (
                            <tr key={slot.id} className={`hover:bg-slate-800/30 transition group ${selected.has(slot.id) ? 'bg-blue-600/5' : ''}`}>
      ...
      <ConfirmDialog
        open={confirmOpen}
        title={`Supprimer les créneaux (${selected.size})`}
