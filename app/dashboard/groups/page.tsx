...
const UI: Record<Lang, {
  ...
  confirmDelete: string; deleteLabel: string; cancelLabel: string; studentsLabel: string
}> = {
  fr: {
  ...
  en: {
    ...
    saving: 'Saving…', update: 'Update Group', add: 'Add Group', cancel: 'Cancel', deleteLabel: 'Delete', cancelLabel: 'Cancel',
    ...
    <div className={`max-w-6xl mx-auto space-y-4 sm:space-y-6 ${isRtl ? 'text-right' : ''}`}>
      ...
      {selected.size > 0 && (
        <div className="bg-blue-600/10 border border-blue-500/30 rounded-xl px-4 py-3 flex flex-wrap items-center justify-between gap-3">
          <p className="text-sm font-medium text-blue-400">{selected.size} groupe{selected.size > 1 ? 's' : ''} sélectionné{selected.size > 1 ? 's' : ''}</p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
                ...
                <tr className="border-b border-slate-800">
                  <th className="px-4 py-3 w-10">
                    <input type="checkbox"
                      ...
                      onChange={() => toggleSelectAll(filtered.map(g => g.id))}
              ...
              <tbody className="divide-y divide-slate-800">
                {filtered.map((group) => (
                  <tr key={group.id} className={`hover:bg-slate-800/40 transition group ${selected.has(group.id) ? 'bg-blue-600/5' : ''}`}>
                    ...
                    <td className="px-4 sm:px-5 py-4">
                      <span className="text-[10px] font-semibold text-white">{group.name}</span>
                    </td>
                    <td className="px-4 sm:px-5 py-4">
      ...
      <ConfirmDialog
        open={confirmOpen}
        title={`Supprimer les groupes (${selected.size})`}
